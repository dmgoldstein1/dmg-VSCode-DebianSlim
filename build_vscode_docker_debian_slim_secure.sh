...
# --- Redirect all verbose output to build log if enabled ---
...
# Usage: ./build_vscode_docker_debian_slim_secure.sh [TunnelName] [--force-analysis] [--logging] [--copilot-is-autonomous] [--git-user-name NAME] [--git-user-email EMAIL] [--use-yaml] [--test-mode]
...
#   --test-mode: Build and start container but exit immediately without waiting for tunnel logs.
...
if ! docker image inspect "$FULL_TAG" >/dev/null 2>&1; then
  ...
  docker build -t "$FULL_TAG" -f "$DOCKERFILE" . | tee "$DOCKER_BUILD_LOG"
...
if [ "$IMAGE_ALREADY_EXISTS" = "false" ]; then
  ...
  if [ "$VERBOSE_LOGGING" = "true" ]; then
    ...
    docker run --rm -it "$FULL_TAG" sh -c 'if command -v npm >/dev/null 2>&1; then find / -type d -name node_modules 2>/dev/null | while read d; do cd "$d" && npm audit fix --force || true; done; fi' | tee "$NPM_AUDIT_LOG" || true
    echo "[INFO] NPM audit log saved to $NPM_AUDIT_LOG"
  else
    docker run --rm "$FULL_TAG" sh -c 'if command -v npm >/dev/null 2>&1; then find / -type d -name node_modules 2>/dev/null | while read d; do cd "$d" && npm audit fix --force || true; done; fi' > "$NPM_AUDIT_LOG" 2>&1 || true
  fi
...
if [ -f "$SCOUT_CVES_LOG" ]; then
  ...
  FIXABLE_COUNT=$(grep -c "fix available" "$SCOUT_CVES_LOG" 2>/dev/null || echo "0")
  ...
  echo "Warning: CVE log not found at $SCOUT_CVES_LOG"
fi
...
if [ "$TEST_MODE" = "true" ]; then
  echo "Test mode enabled - container $CONTAINER_NAME started successfully."
