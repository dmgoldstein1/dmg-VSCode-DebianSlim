...
Supported keys:
- `tunnel_name` – Tunnel/container name (^[a-z0-9_]+$)
- `force_analysis` – Boolean, force re-run of Docker Scout analysis
- `logging` – Boolean, verbose logging for this build script
- `copilot_is_autonomous` – Boolean, enable autonomous Copilot terminal approvals
- `git_user_name` – Git user.name inside the container
- `git_user_email` – Git user.email inside the container
- `gh_token` – GitHub token for non-interactive `gh` login
- `auto_update_on_start` – Boolean-like, control CLI auto-update at startup
- `update_check_interval_seconds` – Seconds between update checks
- `approval_file` – Path inside the container for granting mid-run updates

Tip: The repo already includes `build_variables.yaml` with commented example lines for each key.
